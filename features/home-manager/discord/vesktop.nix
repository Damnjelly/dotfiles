...
}:
{
  config = with config.lib.stylix.colors; {
    ...
    home = {
      packages = with pkgs; [ vesktop rubik ];
      ...
      ''
       ...
       /* @import url('https://refact0r.github.io/system24/src/unrounding.css'); gets rid of all rounded corners. remove if you want rounded corners. */
        ...
        :root {
        	...
        	/* state modifiers */
        	--hover: ${base07}; /* color of hovered elements. */
        	--active: ${base0B}; /* color of elements when clicked. */
        	...
        	--txt-0: ${base0D}; /* color of bright/white text. */
